...
function switchHub(hub) {
  ...
  const modalTitle = document.querySelector("#manualModal h3");
  const modalIdInput = document.getElementById("m-id");
  const modalSpecInput = document.getElementById("m-spec");
  const modalSpecLabel = document.getElementById("modal-spec-label");
  if (modalTitle) {
    modalTitle.innerText =
      hub === "student" ? "Student Registration" : "Staff Registration";
  }
  if (modalIdInput) {
    modalIdInput.placeholder =
      hub === "student" ? "e.g. STG-001" : "e.g. STF-001";
  }
  if (modalSpecLabel) {
    modalSpecLabel.innerText = hub === "student" ? "Specialty" : "Role";
  }
  if (modalSpecInput) {
    modalSpecInput.placeholder =
      hub === "student" ? "e.g. Data Security" : "e.g. Instructor";
  }
  showingDeleted = false;
  const btn = document.getElementById("btn-view-deleted");
  if (btn) btn.innerText = "View Deleted";
  updateUI();
}
...
async function handleLogin() {
  ...
  // FETCH: role and full_name from profiles table
...
function showApp(profile) {
  ...
  if (greetingEl) {
    // UPDATED: Show name if available, otherwise fallback to role
  ...
  actionButtons.forEach((btn) => {
    if (btn.innerText.includes("Reset") && !isAdmin) {
...
window.onload = async () => {
  ...
  if (session) {
    const { data: profile } = await _supabase
      .from("profiles")
      .select("role, full_name") // UPDATED: Include full_name
